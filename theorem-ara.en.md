...
The family $\mathcal{S} = {U_D : D\in\mathcal{D}}$ is taken as a subbase for a topology on $\mathcal{C}$.

We denote by $\mathcal{T}$ the topology generated by $\mathcal{S}$, that is:

```math
\mathcal{T}=\tau(\mathcal{S})=\left\{ \bigcup_{i\in I}\left(\bigcap_{j=1}^{n_i} U_{D_{i,j}}\right) \middle| I\ \text{arbitrary},\ n_i\in\mathbb{N} \right\}.
```
...
$$F_{S1|S2}: \mathcal{C} \rightarrow {S_1, S_2} \text{ where } S_1 \cap S_2 = \emptyset \text{ and } S_1 \cup S_2 = \mathcal{C}$$
...
$$\mathcal{D}_{n+1} = \min_{\delta}[\mathcal{D}_n + \delta] \text{ such that } \tau(\mathcal{D}_{n+1}, \mathcal{D}_n) > 0$$
